...
│
├─ ①b 工具对话兼容改路由（tool_convo_reroute，非限速）
│     若选中 glm51 订阅且 messages 已含 tool_call/tool 消息
│     → 改发 qwen3.7-plus @ codingplan（详见 tool-conversation-reroute.md）
│
...
- **工具对话改路由**：含 tool 历史的 follow-up 若落在 glm51 订阅会强制改路由 Qwen
  （兼容空 200，见 [tool-conversation-reroute.md](./tool-conversation-reroute.md)）。
