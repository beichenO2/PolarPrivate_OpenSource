...
def seed_demo_data(session: Session, vault: VaultService) -> dict:
    """Create or skip the demo project: 1 secret, 1 binding (DashScope VL).
    ...
    session.add(
        Project(
            ...
            description="Demo import (D-110): Aliyun DashScope binding only.",
        )
    )
    ...
    session.add(
        Secret(
            ...
            key="secret.aliyun.dashscope.api_key",
            value=vault.encrypt_secret_value("demo-dashscope-api-key-placeholder"),
            category="proxy",
    ...
    session.add(
        Binding(
            id=str(uuid.uuid4()),
            project_id=project_id,
            service_name="llm.aliyun.dashscope",
            secret_ref_key="secret.aliyun.dashscope.api_key",
        )
    )
